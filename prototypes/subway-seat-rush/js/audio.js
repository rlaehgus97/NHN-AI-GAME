...
const AudioFX = (() => {
  ...
  function play(name){
    const sounds={
      ...
      broth:()=>{ tone(310,.22,{type:'triangle',to:220,volume:.09}); noise(.3,.05,1300); },
      brothWarn:()=>{ tone(680,.16,{type:'square',to:520,volume:.07}); },
      climate:()=>{ noise(.7,.065,2400); tone(185,.55,{type:'sine',to:145,volume:.05}); },
      umbrella:()=>{ noise(.5,.07,1700); tone(420,.2,{type:'triangle',to:310,volume:.05}); },
      water:()=>{ noise(.55,.09,1500); tone(260,.35,{type:'sine',to:170,volume:.04}); },
      repair:()=>{ tone(440,.16,{volume:.07}); setTimeout(()=>tone(660,.22,{volume:.07}),150); },
