...
(function(){
  ...
  function isAlternatingSeatPattern(actions){
    ...
    return recent.every((action,index)=>index===0 || action.type!==recent[index-1].type);
  }
  ...
  function evaluateSeatingConstraints(state,snapshot){
    ...
    if(snapshot.posture===Posture.SEATED &&
      ...
      actions.push({type:'FORCE_STAND',reason:'continuous-seated',
        seatedFor:AI_DIRECTOR.longSeatDuration,cooldownUntil:state.longSeatCooldownUntil});
      return actions;
    }
    ...
    if(state.recentSeatActions.length>=AI_DIRECTOR.rapidToggleTransitions &&
      ...
      actions.push({type:'LOCK_SEATING',reason:'rapid-seat-toggle',
        lockUntil:state.seatingLockedUntil,cooldownUntil:state.rapidToggleCooldownUntil});
    }
    return actions;
  }
  ...
  class EventDirector {
    ...
    isSeatingLocked(time=this.elapsed){ return time<this.behavior.seatingLockedUntil; }
    update(dt,snapshot={}){
      this.elapsed += dt;
      const actions = [];
